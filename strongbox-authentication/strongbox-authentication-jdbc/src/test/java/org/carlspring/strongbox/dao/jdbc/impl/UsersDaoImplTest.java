...
public class UsersDaoImplTest
{
            ...
            throws Exception
    {
        ...
        final long countOld = usersDaoJdbc.count();

        usersDaoJdbc.createUser(user);

        final long countNew = usersDaoJdbc.count();
        ...
        usersDaoJdbc.updateUser(user);

        final User updatedUser = usersDaoJdbc.findUser(USERNAME, changedPassword);
        ...
        assertFalse("This user is already an administrator!", usersDaoJdbc.hasRole(user.getUsername(), roleName));

        usersDaoJdbc.assignRole(user, roleName);

        assertTrue("Failed to assign role 'ADMINISTRATOR' to user '" + USERNAME + "'",
                   usersDaoJdbc.hasRole(user.getUsername(), roleName));

        // Delete the user
        usersDaoJdbc.removeUser(user);
    }
